...
export class TasksService {
  ...
  async updateTaskById(
    ...
    // Recalculate priority if deadline or status is updated
    const deadline = taskDto.deadline
      ? new Date(taskDto.deadline)
      : existingTask.deadline;
    const status = taskDto.status ? taskDto.status : existingTask.status;
